...
export type Store = {
  ...
  userKey: string;
  destroy: () => void,
  sendSocketServerRoundTripRequest: <T, U>(event: string, args?: T, resultEvent?: string) => Promise<U>;
...
export default makeStore<Store>('socketStore', () => {
  ...
  const sendSocketServerRoundTripRequest = async <T, U>(event: string, args?: T, resultEvent?: string): Promise<U> => {
