<template>
  <label><span>キャラクターシート倉庫URL</span><input type="text" v-model="data.sheetInfo.url"></label>
  <label><span>名前</span><input type="text" v-model="data.sheetInfo.characterName"></label>
  <label><span>名前カナ</span><input type="text" v-model="data.sheetInfo.characterNameKana"></label>
  <label><span>上位流派</span><input type="text" v-model="data.sheetInfo.upperStyle"></label>
  <label><span>下位流派</span><input type="text" v-model="data.sheetInfo.subStyle"></label>
  <label><span>流儀</span><input type="text" v-model="data.sheetInfo.stylerule"></label>
  <label><span>仇敵</span><input type="text" v-model="data.sheetInfo.foe"></label>
  <label><span>階級</span><input type="text" v-model="data.sheetInfo.level"></label>
  <label><span>年齢</span><input type="text" v-model="data.sheetInfo.age"></label>
  <label><span>表の顔</span><input type="text" v-model="data.sheetInfo.cover"></label>
  <label><span>PC番号</span><input type="text" v-model="data.sheetInfo.scenario.pcno"></label>
  <label><span>功績</span><input type="text" v-model="data.sheetInfo.exp"></label>
  <label><span>性別</span><input type="text" v-model="data.sheetInfo.sex"></label>
  <label><span>信念</span><input type="text" v-model="data.sheetInfo.belief"></label>
  <table class="skill">
    <tr>
      <template v-for="(s, col) in skillColumnList" :key="s">
        <th :class="`gap-${col}`">
          <input type="checkbox" :checked="data.sheetInfo.tokugi.spaceList.some(n => n === col)">
        </th>
        <th :class="`skill-${col}`">
          {{s}}<input type="checkbox" :checked="data.sheetInfo.tokugi.damagedColList.some(n => n === col)">
        </th>
      </template>
      <th class="row-num"></th>
    </tr>
    <tr class="row-2" v-for="(tList, row) in tokugiTable" :key="row">
      <template v-for="(t, col) in tList" :key="t">
        <td :class="`gap-${col}`">
          <input type="checkbox" :checked="data.sheetInfo.tokugi.spaceList.some(n => n === col)">
        </td>
        <td :class="[`skill-${col}`, data.sheetInfo.tokugi.learnedList.some(n => n.column === col && n.row === row)? 'learned' : '']">
          {{ t }}
        </td>
      </template>
  ...
  <table class="ninja-arts">
    <tr>
      ...
      <th class="page">参照p</th>
    </tr>
    <tr v-for="(n, ind) in data.sheetInfo.ninpouList" :key="ind">
      <td class="secret">{{n.secret}}</td>
      ...
      <td class="effect">{{n.effect}}</td>
      <td class="page">{{n.page}}</td>
    </tr>
  </table>
...
import { ShinobigamiHelper, tokugiTable } from '@/components/the-play/ope-part/shinobigami'
...
export default defineComponent({
  ...
  setup() {
    ...
    let pcNo = 0
    characterStore.characterList.value
    ...
    watch(() => data.sheetInfo.url, async () => {
      ...
      const pcNoRaw = Number(rd.scenario.pcno.replace(/[０-９]/g, s => String.fromCharCode(s.charCodeAt(0) - 0xFEE0)))
      if (!Number.isNaN(pcNoRaw)) data.pcNo = pcNoRaw
      data.sheetInfo = rd
    })
