  ...
  setup(props) {
    ...
    watch(() => props.r.status, () => {
      ...
      if (status === 'initial-touched') {
        setTimeout(() => { roomNameInputElm.value?.focus() }, 50)
      }
    ...
    watch([userStoreInjectObj.lastRoomLoginType, userStoreInjectObj.selectedRoomNo], () => {
      if (userStoreInjectObj.selectedRoomNo.value !== props.r.roomNo) return
      const type = userStoreInjectObj.lastRoomLoginType.value
      if (type === '') {
        setTimeout(() => { roomPasswordInputElm.value?.focus() }, 50)
      }
      if (type === 'login') {
        setTimeout(() => { userNameInputElm.value?.focus() }, 50)
      }
